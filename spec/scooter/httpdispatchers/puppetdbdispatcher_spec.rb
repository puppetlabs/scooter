    ...
    context 'with a beaker host passed in' do
      ...
      describe '.database_matches_self?' do
        ...
        it 'compare with self' do
          expect{subject.database_matches_self?('test.com')}.not_to raise_error
        end

        it 'compare with different' do
          expect{subject.database_matches_self?('test2.com')}.to raise_error /Nodes do not match - other_nodes/
        end
      end
